...
describe "Behaviour of EndpointsProxy" do
	...
	before(:all) do
		...
		@url = "https://beta.openphacts.org/search/byTag?app_id=#{OPS_API_ID}&app_key=#{OPS_API_KEY}&q=xxxx&limit=5&uuid=eeaec894-d856-4106-9fa1-662b1dc6c6f1&_format=json"
		@options ={
			:q => "some",
			:query => "some",
			:limit => 25,
	...
	describe "Remote endpoints behaviour..." do
		it "module check conceptWiki should ping back" do
			...
			OpsEndpointsProxy.check_conceptwiki.should be_true
			OpsEndpointsProxy.check_conceptwiki.should satisfy { |it|
		...
		it "check OPS API should ping back" do
			ops_api_alive = OpsEndpointsProxy.check_ops_api
			ops_api_alive.should be_true

			# core_endpoint = OpsEndpointsProxy.get_core_endpoint
		...
		it "make_request should get a response" do
			...
			api_alive = OpsEndpointsProxy.check_ops_api
			api_alive.should be_true
			# OpsEndpointsProxy.get_core_endpoint.should_not be_nil

			url = @url.gsub(/xxxx/, @options[:q])
			res = OpsEndpointsProxy.make_request(url, @options)
...
#			res.code.to_i.should be == 200
		...
		it "uniprot2json should return a json array from tab rows" do
			json_str = OpsEndpointsProxy.uniprot2json(@tab_string, 'brca2')
...
#				line = line.gsub(/"*</, "&lt;").gsub(/"*>./, "&gt;")
		...
		it "should parse the content" do
			result = OpsEndpointsProxy.buildup_uniprot_info(@xmlContent)
