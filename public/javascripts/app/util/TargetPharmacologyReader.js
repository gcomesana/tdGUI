...
Ext.define('TDGUI.util.TargetPharmacologyReader', {
  ...
  readRecords: function (data) {
    ...
    //big chunk of data
    var result = data[TDGUI.util.LDAConstants.LDA_RESULT];
    var page_uri = result[TDGUI.util.LDAConstants.LDA_ABOUT];
    var next_page = result[TDGUI.util.LDAConstants.LDA_PAGINATED_NEXT];
    var previous_page = result[TDGUI.util.LDAConstants.LDA_PAGINATED_PREVIOUS];
    var page_size = result[TDGUI.util.LDAConstants.LDA_PAGINATED_PAGE_SIZE];
    var start_index = result[TDGUI.util.LDAConstants.LDA_PAGINATED_START_INDEX];

    var items = result[TDGUI.util.LDAConstants.LDA_ITEMS];
    if (items == null) {
      Ext.Error.raise({
    ...
    Ext.each(items, function (item, index, items) {
      ...
      //big bits
      var forMolecule = item[TDGUI.util.LDAConstants.LDA_FOR_MOLECULE];
      ...
      if (forMolecule != null) {
        chembl_compound_uri = forMolecule[TDGUI.util.LDAConstants.LDA_ABOUT];
        ...
        em = forMolecule[TDGUI.util.LDAConstants.LDA_EXACT_MATCH];
      }
      ...
      Ext.each(em, function (match, index, matches) {
          ...
          if (TDGUI.util.LDAConstants.LDA_SRC_CLS_MAPPINGS[src] == 'conceptWikiValue') {
            cw_compound_uri = match[TDGUI.util.LDAConstants.LDA_ABOUT];
            ...
            cs_src = match[TDGUI.util.LDAConstants.LDA_IN_DATASET];
            ...
            drugbank_src = match[TDGUI.util.LDAConstants.LDA_ABOUT];
          }
        }
      );

      var onAssay = item[TDGUI.util.LDAConstants.LDA_ON_ASSAY];
      ...
      if (onAssay != null) {
        chembl_assay_uri = onAssay[TDGUI.util.LDAConstants.LDA_ABOUT];
      ...
      if (target != null) {
        chembl_target_uri = target[TDGUI.util.LDAConstants.LDA_ABOUT];
        //target_pref_label = target['prefLabel'];

        targetMatch = target['exactMatch'];
        if (targetMatch != null) {
          var targetMatchURI = targetMatch[TDGUI.util.LDAConstants.LDA_ABOUT];
      ...
      var record = Ext.create('TDGUI.model.lda.TargetPharmacologyModel', {
        //for page
        page_uri: page_uri,
        next_page: next_page,
