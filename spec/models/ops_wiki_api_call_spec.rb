...
#		res = @conceptWiki.request(@url, @opts)
		...
		res = @ops_wiki_api_call.search_by_tag(@opts_target[:uuid],
																					 @opts_target[:q], {})
		res_bis = @ops_wiki_api_call.search_by_tag(@opts_target[:uuid],
	...
	it "should return a list of compounds for penicillin" do
		res = @ops_wiki_api_call.search_by_tag(@penicillin_cmpd[:uuid], @penicillin_cmpd[:query], {})
		res.should be_kind_of(Array)
